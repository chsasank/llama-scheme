...
(import scheme)
(import (chicken base) (chicken file) (chicken port)
        (chicken syntax) (chicken fixnum))
...
(define compiler-ext (or (memq #:compiling ##sys#features) (memq #:compiler-extension ##sys#features)))

(define register-foreign-type! 
  (if compiler-ext
      (##sys#slot 'chicken.compiler.support#register-foreign-type! 0)
      void))

(define lookup-foreign-type
  (if compiler-ext 
      (##sys#slot 'chicken.compiler.support#lookup-foreign-type 0)
      void))

(define debugging-chicken
  (if compiler-ext
      (##sys#slot 'chicken.compiler.support#debugging-chicken 0)
      '()))

;; ... end of terrible hack

(define (test-debug-flag sym)
  (memq sym debugging-chicken))
...
(define parse-type-rec
  (match-lambda
    ...
    [`(u8 . ,more) (values 'unsigned-byte more)]
    [`(s8 . ,more) (values 'byte more)]
...
(define usual-naming-transform
  (let ()
    ...
    (lambda (m)
      (downcase-string
       (string-translate 
	(irregex-replace/all "([a-z])([A-Z])" (car m) 
                             (lambda (m) 
...
(define (parse-type-declaration vals r)
  (let rec ([vals vals])
    (match vals
	 ...
	 (register-foreign-type! stname stype) ; will be overwritten later
	 `(,(r 'begin)
	   (,(r 'foreign-declare)
...
(define (foreign-type->class ftype io)
  (let rec ([ftype ftype])
    (match ftype
      ...
      (('template . _) '<pointer>)
      [(? symbol?)
       (let ([a (lookup-foreign-type ftype)])
	 (if a
