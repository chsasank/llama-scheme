...
(import (chicken pathname) (chicken io) (chicken base) (chicken port))
(import regex (chicken process-context))
  ...
  (let ((files '())
	...
	(let ((arg (car args))
	  ...
	  (cond ((string=? "-debug" arg)
		 (set! debugging-chicken '(C X))
		 (loop rest))
		((string=? "-export-constants" arg)
		 (set-bind-options export-constants: #t)
		 (loop rest))
		((string=? "-class-finalizers" arg)
		 (set-bind-options class-finalizers: #t)
		 (loop rest))
    ...
    (for-each
     (lambda (f)
       (define (read-from-file-or-stdin)
         (if (string=? f "-")
	 ...
	 (pp `(begin
		,@(parse-easy-ffi (read-from-file-or-stdin)
                                  identity
                                  chunkify-only
                                  f)))
	 (print "\n;;; END OF FILE"))
